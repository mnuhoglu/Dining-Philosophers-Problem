...
public class Philosopher extends Thread {
	...
	public void run() {
		...
		int Sleeping = new Random().nextInt(1000);   //philosophers arrive the table between time period 1-10 secs
		...
		try {                                         //every philosopher waited to start.
			for (int i = 0; i < N; i++) {
				if (id != i) {
					barriers[i].release();
				}
		...
		while(true)
		{
			...
			table.StopEating_GUI(id);                // philosophers stop eating in order to let others eat.
		}
	}

	public void Thinking(int i) throws InterruptedException {//philosopher thinks random amount of time.
		int s =new Random().nextInt(1000);
		Thread.sleep(s);

	}

	public void TakeFork(int i) throws InterruptedException {//
		mutex.acquire();
		state[i] = HUNGRY;
		table.Hungry_GUI(i);
		tester(i); //left
	...
	public void PutForks(int i) throws InterruptedException {//philosopher puts the forks after eating.
		mutex.acquire();
		state[i]=THINKING;
		table.ForkPut_GUI(i);
		tester((i+N-1)%N); //left
	...
	public void tester(int i) {  //checks if the philosopher is able to eat
